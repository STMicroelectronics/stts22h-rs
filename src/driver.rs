use super::{
    BusOperation, I2c, RegisterOperation, SensorOperation, SevenBitAddress, bisync, i2c, prelude::*,
};
...
#[bisync]
impl<P> Stts22h<i2c::I2cBus<P>, OnState>
where
    P: I2c,
{
...
#[bisync]
impl<B, S> SensorOperation for Stts22h<B, S>
where
    B: BusOperation,
    S: SensorState,
{
    ...
    #[inline]
    async fn write_to_register(&mut self, mut reg: u8, buf: &[u8]) -> Result<(), Error<B::Error>> {
...
#[bisync]
impl<B: BusOperation> Stts22h<B, OnState> {
    ...
    pub async fn temp_data_rate_set(&mut self, val: OdrTemp) -> Result<(), Error<B::Error>> {
        let mut ctrl = Ctrl::read(self).await?;
    ...
    pub async fn temp_data_rate_get(&mut self) -> Result<OdrTemp, Error<B::Error>> {
        let ctrl = Ctrl::read(self).await?;
    ...
    pub async fn block_data_update_get(&mut self) -> Result<u8, Error<B::Error>> {
        Ctrl::read(self).await.map(|reg| reg.bdu())
    }
    ...
    pub async fn temperature_raw_get(&mut self) -> Result<i16, Error<B::Error>> {
        TempOut::read(self).await.map(|reg| reg.temperature())
    }
    ...
    ///     * `u8`: WHO_AM_I content
    ///     * `Err`: Returns an error if the operation fails.
    pub async fn dev_id_get(&mut self) -> Result<u8, Error<B::Error>> {
    ...
    ///     * `Err`: Returns an error if the operation fails.
    pub async fn dev_status_get(&mut self) -> Result<DevStatus, Error<B::Error>> {
        let status = Status::read(self).await?;
    ...
    pub async fn smbus_interface_set(&mut self, val: SmbusMd) -> Result<(), Error<B::Error>> {
        let mut ctrl = Ctrl::read(self).await?;
    ...
    ///     * `Err`: Returns an error if the operation fails.
    pub async fn smbus_interface_get(&mut self) -> Result<SmbusMd, Error<B::Error>> {
        let ctrl = Ctrl::read(self).await?;
    ...
    pub async fn auto_increment_get(&mut self) -> Result<u8, Error<B::Error>> {
        Ctrl::read(self).await.map(|reg| reg.if_add_inc())
    }
    ...
    pub async fn temp_trshld_high_set(&mut self, val: u8) -> Result<(), Error<B::Error>> {
        TempHLimit::new().with_thl(val).write(self).await
    }
    ...
    pub async fn temp_trshld_high_get(&mut self) -> Result<u8, Error<B::Error>> {
        TempHLimit::read(self).await.map(|reg| reg.thl())
    }
    ...
    pub async fn temp_trshld_low_set(&mut self, val: u8) -> Result<(), Error<B::Error>> {
        TempLLimit::new().with_tll(val).write(self).await
    }
    ...
    pub async fn temp_trshld_low_get(&mut self) -> Result<u8, Error<B::Error>> {
        TempLLimit::read(self).await.map(|reg| reg.tll())
    }
    ...
    /// * `Err`: Returns an error if the operation fails.
    pub async fn temp_trshld_src_get(&mut self) -> Result<TempTrlhdSrc, Error<B::Error>> {
        let status = Status::read(self).await?;
